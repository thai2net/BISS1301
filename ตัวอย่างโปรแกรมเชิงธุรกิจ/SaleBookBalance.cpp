// SALES JOURNAL BALANCE SHEET -- SaleBookBalance.cpp
#include <stdio.h>
#include <string.h>
#include <iostream>
#include <iomanip>
...
int main(){
   ...
   AccountBOOK ac_book2019[row], *p; // ประกาศตัวแปรโครงสร้างและพอนย์เตอร์
    ...
    cout << setw(60) << "SALES JOURNAL BALANCE SHEET" << endl;
    for(int i = 0; i < 120; i++){cout << '-';}cout << endl;
    cout << setw(15) << title_row_book[0];
    ...
    for(int i = 0; i < row; i++){ // แถวรายการสมุดรายวันขั้นต้น
        string book = ac_book2019[i].book;
        if(book == "Sale"){ // เลือกเฉพาะรายการขาย
